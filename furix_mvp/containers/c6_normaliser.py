...
KW = {
    ...
    "Control 2":  r"\b7045\b|new service was installed|service installed|unauthorized software|unknown executable|installutil|new service|apt[\- ]?(get )?install|\bdpkg\b|yum install|package",
    "Control 3":  r"getsecretvalue|getsecret|\bexfil|exfiltrat|sensitive (data|file|employee)|secretsmanager|credential dump|sekurlsa|redirectmessageto|auto[- ]?forward|\ballusers\b|publicly accessible",
    "Control 4":  r"conditional access|legacy authentication|default password|certificate has expired|cert(ificate)? expired|misconfig|insecure config(uration)?|weak cipher|sslv3|policy.{0,20}disabled|disabled.{0,20}policy",
    "Control 5":  r"\buseradd\b|net user .*\/add|createuser|create user|new[- ]?localuser|\b4720\b|\b4732\b|provision\.?user|add member to role|account created|\badduser\b|createserviceaccount|createaccesskey|service account|privilege\.grant|account\.privilege",
    "Control 6":  r"\bsudo\b|privilege escalat|\bescalat|sebackup|\b4672\b|failed password|invalid user|accepted password|\b4625\b|\b4732\b|mfa.{0,20}(reset|deactivat|disabl|bypass|fail)|mfaused.{0,4}no|without mfa|no mfa|conditional access|roles/owner|global admin|super admin(istrator)?|administratoraccess|administrator access|attachuserpolicy|localgroup admin|group administrators|authorized_keys",
    "Control 7":  r"\bcve-\d|\bnmap\b|vulnerab|\bexploit|\bvuln\b",
    "Control 8":  r"\bauditd\b|\bexecve\b|\b4688\b|\b4698\b|integrity (checksum|changed)|checksum changed|type=syscall|/etc/shadow|audit log|auditctl",
    "Control 9":  r"\bwget\b|\bcurl\b|malicious-domain|malware-c2|phishing|clicked .{0,30}(url|link)|\.ru\b|\bexfil|data-exfil|payload\.(sh|ps1)",
    "Control 10": r"mimikatz|\bbeacon\b|cobalt(strike)?|ransom|\.locked\b|encrypt\.exe|powershell\s+-?enc|powershell.{0,20}-enc|\bmalware\b|/payload\b|payload\.(sh|ps1|exe)",
    "Control 11": r"deletebucket|delete bucket|backup.{0,15}(delet|fail)|delet.{0,15}backup|snapshot delet|\bransom|no restore point|recovery (fail|point)",
    "Control 12": r"\bfirewall\b|\bufw\b|%asa|access-list|access-group|vpn (config|gateway|tunnel|policy)|network infrastructure",
    "Control 13": r"suricata|\bzeek\b|intrusion detection|intrusion prevention|\bids/ips\b|\bnids\b|\bbeacon\b|eternalblue|signature_id|et (exploit|malware|scan)|c2 checkin|command and control|dns tunnel|\bTXT\b query|long-duration",
    "Control 14": r"phishing|clicked .{0,30}(url|link|email)|social engineering|security awareness|awareness training",
    "Control 15": r"\biam\b|service account|gserviceaccount|createserviceaccount|consolelogin|\bcontractor\b",
    "Control 16": r"sql injection|\bsqli\b|modsecurity|\bwaf\b|remote code execution|\brce\b|\bxss\b|command injection|directory traversal",
    "Control 18": r"penetration test|\bpentest\b|red team|authorized scan|security assessment window",
}
